...
## 핵심 약속: 수동적으로 읽고 끝낼 수 없다

새 개념을 설명한 뒤에는 반드시 정답을 가린 회상 질문이 이어집니다. 사용자가 `알겠어`, `다음`, `계속`이라고만 해도 바로 다음 개념으로 넘어가지 않습니다.
...
사용자가 명시적으로 복습을 건너뛸 수는 있지만, 회상하지 않은 개념을 외웠다고 기록하지 않습니다.
...
| 3 | 힌트 없이 자기 말로 회상함 |
| 4 | 새로운 사례에 적용함 |
| 5 | 다음 간격 복습에서도 독립적으로 회상함 |

3점 전에는 새 개념을 추가하지 않는 것이 기본 규칙입니다. 같은 대화에서 3점 이상이면 `오늘 통과`, 새로운 사례까지 풀면 `적용 가능`, 이후 복습에서도 기억하면 `장기 기억 확인`으로 구분합니다.
...
- 독립 회상 전에는 새 개념으로 넘어가지 않는 Mastery Gate
- 실패 시 힌트→재회상→적용으로 이어지는 적응형 반복
